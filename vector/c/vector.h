...
// inspired on the implementation by Edd mann(https://eddmann.com/posts/implementing-a-dynamic-vector-array-in-c/)
...
void resize(vector *v, int new_capacity); // self explanatory
...
void check_increase(vector *v); // Checks if the size of the vector is on the capacity limit, increases the capacity
void check_decrease(vector *v); // Checks if the size of the vector in below (capacity / 4), then decreases the capacity
